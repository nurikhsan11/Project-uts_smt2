...
if ($_proses == "Simpan") {
   // data baru
   $sql = "INSERT INTO produk (kode, nama, harga_jual, harga_beli, stok, min_stok, deskripsi, kategori_produk_id, foto) VALUES (?,?,?,?,?,?,?,?,?)";
} else if ($_proses == "Update") {
   $ar_data[] = $_POST['idedit'];
   $sql = "UPDATE produk SET kode=?, nama=?, harga_jual=?, harga_beli=?, stok=?, min_stok=?, deskripsi=?, kategori_produk_id=?, foto=? WHERE id=?";
   // if (isset($_foto['name']) && !empty($_foto['name'])) {
   //    $sql .= ", gambar=?";
   // }
...
header('location: ../index.php?krupuk=table_produk/produk');